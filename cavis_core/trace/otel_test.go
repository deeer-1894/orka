...
func TestStartSpan_RecordsSpanWithTraceID(t *testing.T) {
	...
	// cavis trace id stamped
	var found bool
	for _, a := range spans[1].Attributes() {
		if a.Key == "cavis.trace_id" && a.Value.AsString() == "abc123" {
			found = true
		}
	}
	if !found {
		t.Fatalf("cavis.trace_id attribute missing: %v", spans[1].Attributes())
	}
}

func TestInit_NoopWhenUnconfigured(t *testing.T) {
	shutdown, err := Init(context.Background(), "cavis-test")
	if err != nil {
		t.Fatal(err)
	}
