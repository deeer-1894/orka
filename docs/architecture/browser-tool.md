...
| `fill_form` | 顺序填写至多12个字段，返回完成数量和失败位置，不主动点击提交 |
...
`snapshot_id` 现在表示文档/运行/GUI代次范围。普通输入和后续快照保留未变化节点的引用；节点被替换、语义属性改变或iframe导航后，旧引用拒绝执行。不会按同名节点猜测替代目标。需要自动重新定位的响应式页面可以使用唯一CSS选择器。

`fill_form.fields` 每项提供 `ref` + `snapshot_id` 或 `selector`，以及二选一的 `text`（输入）/`value`（选择）；总输入最多16KiB，最多12项。不会发送提交动作，但网页自身的input/change处理器仍可能产生外部副作用。失败立即停止，`form.completed` 是已确认完成的字段数，`failed_index` 从0开始；先前动作不会回滚或重放。整个fields在显示/历史参数中脱敏，实际执行及确认检查点保留原值。

动作在派发前对缺失、隐藏、禁用或遮挡目标最多等待约1.5秒，同时受请求取消/超时约束；参数歧义、旧引用、跨域边界和已派发后的错误不会重试。点击/按键派发未完整确认返回 `outcome_unknown`。

### 观察与变化

页面定位在 `scripts/dom.js`，快照与动作脚本在 `scripts/snapshot.js`，有界变化比较在 `scripts/observation.js`。观察基线只存于会话页面隔离世界，运行或GUI代次变化重置；不在Go全局保存页面状态。快照保留元素允许的动作、选项、框架路径、文档ready_state。

首次观察、导航、显式snapshot、`view=full`、内容过多或顺序变化无法用文本集合差量表达时，返回full。其他动作默认返回delta/unchanged文本，并保留全部当前有界可操作元素。`change`列出新增/移除文本（最多各24条、每条240字符，省略显式标记）。无可见变化并不证明动作未发生；相同目标连续无变化时 `progress` 提醒检查状态，不阻止任务。页面效果异步完成时应使用明确条件的wait再观察。
...
- 操作主文档、开放的Shadow DOM及可访问同源iframe。CSS目标可带最多4层frame选择器路径，快照引用自动绑定所在文档；帧被覆盖或坐标变换不明时不猜测点击位置。跨域/不可访问iframe返回原因及GUI接力提示，仍不授予跨来源访问；GUI接力后旧DOM引用失效，应重新观察。弹窗管理及纯像素内容仍需要GUI或后续支持。
