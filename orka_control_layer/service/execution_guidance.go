...
const verificationGuidance = "Follow the original user requirements. For implementation or data deliverables, write and run a real task-specific verifier as soon as the first working artifact exists; derive assertions independently from the requirements, not constants copied from generated results. Check applicable business conservation, group-to-overall consistency (including period-end aggregation), and boundary cases before charts and reports. For CSV-based report counts, totals and ranges, use find_tools to locate render_report, bind the numbers to CSV columns in a .report.json spec, and declare both spec and Markdown output with update_plan.outputs so check_delivery detects stale values. Keep template literals factual: the binding checker does not validate unbound prose, explanations or citations. Validate narrative claims against the exact source rows, distinguishing discarded from retained records and observations from causal hypotheses. Charts must preserve a common numeric scale; inspect rendered output when available. Repair failed assertions, rerun affected checks, and retain actual command results. Complete one small executable action per generation; do not bundle the entire project into one call. Reuse saved findings; reread unchanged source files only for a specific unresolved question. This guidance and check_delivery are not proof of semantic correctness; never claim verification without executed checks and their evidence. "
