	...
	return db.Workflow{
		...
		Steps: []db.WorkflowStep{
			...
			},
			{
				...
				Prompt:    "这是第一次独立提取(双盲A)。委托 factor_proposer 把下面的投资逻辑转成已通过 validate_factor 校验的因子,只输出 JSON 数组:\n{{parse}}",
				OnError:   "retry:1",
			},
			{
				Name:      "propose_b",
				DependsOn: []string{"parse"},
				Prompt:    "这是第二次独立提取(双盲B),请独立思考、不要照抄。委托 factor_proposer 把下面的投资逻辑转成已通过 validate_factor 校验的因子,只输出 JSON 数组:\n{{parse}}",
				OnError:   "retry:1",
			},
			{
				Name:      "agree",
				DependsOn: []string{"propose_a", "propose_b"},
				Prompt: "调用 factor_agreement,set_a={{propose_a}},set_b={{propose_b}}。只保留 agreement(一致性)达标(>=" + factorAgreementThreshold +
					")的稳定因子(用两次都出现的版本);低一致性的因子在结尾用一行 `LOW_AGREEMENT: <名称列表>` 标出。最终只输出保留下来的因子 JSON 数组。",
			},
			{
				Name:      "evolve",
...
func (s *ChatService) RunFactorPipeline(ctx context.Context, owner string) []string {
	...
	for _, rp := range reports {
		cid := s.runOneReport(ctx, owner, rp)
		if cid != "" {
...
// runOneReport runs the pipeline for a single report with panic isolation.
func (s *ChatService) runOneReport(ctx context.Context, owner, reportPath string) (cid string) {
	defer func() {
		if r := recover(); r != nil {
			if s.Log != nil {
	...
	return s.RunWorkflow(ctx, wf, messages.NewID())
}
