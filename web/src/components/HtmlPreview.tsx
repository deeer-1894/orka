...
export function HtmlPreview({ text, name, conv }: { text: string; name: string; conv: string }) {
  ...
  useEffect(() => {
    ...
    prepareHtmlPreview(text, name, path => fetch(files.downloadURL(path, conv), { headers: { Authorization: 'Bearer ' + auth.token() }, signal: controller.signal }), controller.signal)
  ...
  return <>
    ...
    <iframe title={`HTML 页面：${name}`} sandbox="allow-scripts" referrerPolicy="no-referrer" srcDoc={preview.document} className="h-[70vh] min-h-[280px] w-full border-0 bg-white" />
  </>;
}
