...
export default function RunBudgetPanel({
  runID
}: {
  runID: string;
}) {
  ...
  return <section aria-label="运行预算" className="space-y-2 rounded-lg border border-border p-3 text-xs">
    <div className="flex justify-between"><strong>运行预算</strong><ActionChip onClick={() => setRefresh(n => n + 1)} icon="refresh">刷新预算</ActionChip></div>
    {error && <p role="alert" className="text-accent">预算读取失败：{error}</p>}
    {!data && !error && <p role="status">正在读取预算…</p>}
    {data && <>
      {data.budget_run_id !== data.run_id && <p className="break-all">共享预算：{data.budget_run_id}</p>}
      <p>已用（含保守占用）{quantity(data.used_tokens)} · 预留 {quantity(data.reserved_tokens)} · 剩余 {quantity(data.remaining_tokens)} tokens</p>
      ...
      <p>上限：{quantity(data.limits.max_tokens)} tokens · {quantity(data.limits.max_wall_seconds)} 秒 · {quantity(data.limits.max_steps)} 轮</p>
      <p>已用轮次：{quantity(data.used_steps)} · 截止：{data.deadline || '未提供'}</p>
