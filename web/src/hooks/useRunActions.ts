...
import { api } from '../api';
import { RunActions } from '../lib/runActions';
import type { RunRecord } from '../types';
import { refreshResource } from '../lib/useResource';
export function useRunActions(attach: (cid: string) => unknown) {
 const [, update] = useState(0);
 const ref = useRef(attach); ref.current = attach;
 const [actions] = useState(() => new RunActions({ list: async cid => (await api.listRuns({conversation_id:cid})).runs || [], get: api.getRun, resume: api.resumeRun }, () => update(n => n + 1)));
