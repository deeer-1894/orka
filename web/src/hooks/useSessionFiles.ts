...
export function useSessionFiles(messages: Message[], context: FileContext, status: string) {
  const revision = useFileRevision(context.conversationID);
  ...
  const key = JSON.stringify([context.conversationID, context.ownerEmail, status, revision, evidence?.id, candidates]);
